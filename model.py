...
class CAN(nn.Module):
    ...
    def encode(self, x,adj,sim):
        '''在forward中调用的layer的参数传到对应layer的forward处'''

        z_u = F.relu(self.hidden1(x,torch.add(adj,sim*0.3)))
    ...
    def forward(self, features,adj,sim):
        z_u_mean,z_u_log_std,z_a_mean,z_a_log_std = self.encode(features,adj,sim)
